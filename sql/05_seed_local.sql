-- ============================================================
-- DATOS INICIALES - Versión Local
-- ============================================================

-- Usuario admin de prueba (password: admin123)
INSERT INTO auth.users (email, password_hash) VALUES
('admin@herrajes.local', 'hash_admin123'),
('ventas@herrajes.local', 'hash_ventas123'),
('bodega@herrajes.local', 'hash_bodega123'),
('compras@herrajes.local', 'hash_compras123'),
('consulta@herrajes.local', 'hash_consulta123');

-- Perfiles de usuario
INSERT INTO perfil_usuario (user_id, nombre_completo, rol, telefono) VALUES
((SELECT id FROM auth.users WHERE email = 'admin@herrajes.local'), 'Admin Sistema', 'admin', '81-1000-0001'),
((SELECT id FROM auth.users WHERE email = 'ventas@herrajes.local'), 'Ventas Ejecutivo', 'ventas', '81-1000-0002'),
((SELECT id FROM auth.users WHERE email = 'bodega@herrajes.local'), 'Bodega Almacén', 'bodega', '81-1000-0003'),
((SELECT id FROM auth.users WHERE email = 'compras@herrajes.local'), 'Compras Proveedor', 'compras', '81-1000-0004'),
((SELECT id FROM auth.users WHERE email = 'consulta@herrajes.local'), 'Consulta Reportes', 'consulta', '81-1000-0005');
